...
const asyncHandler =
  (fn: typeof Function) => (req: any, res: any, next: any) => {
...
const setErrorHandler = () => {
  ...
  app.use((err: Error, req: Request, res: Response, next: typeof Function) => {
    ...
    res.status(SERVER_ERROR).json({
      message:
        "Internal Server Error, Please Email SmurfJs for more details...!",
...
const ReadApi = async (err: any, files: string[]) => {
  ...
  for (const api of endpoints) {
    ...
    const setValidation =
      validation === null
        ? (req: Request, res: Response, next: typeof Function) => next()
    ...
    const guardMiddleware = guards.map((row: typeof Function) =>
      asyncHandler(row)
    );
    const policyMiddleware = policies.map((row: typeof Function) =>
      asyncHandler(row)
    );
...
const setAppUse = async () => {
  ...
  for (const row of appUse) {
    ...
    try {
      ...
      const errMsg = `Error in ${name} App Use Settings: ${
